...
void read(int const & a) {
    [[maybe_unused]] int val = a; // = a = 2 змінює const-посилання!
}
...
int main() {
    ...
    int a = 1, b = 2;
    int const *i = &a;
    // *i = 5; не можна змінювати значення через pointer to const
    i = &b;
    ...
    *j = 5;
    ...
    // try constant arguments of functions with references
    {
      int p = 0;
      const int r = 0;
      // write(2); не можна передати літерал як неконстантну reference
